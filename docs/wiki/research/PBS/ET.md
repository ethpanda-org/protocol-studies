...
> [!WARNING]
> 本文档涵盖了一个活跃的研究领域，随着 ePBS、ET 和 Inclusion Lists (IL) 设计空间的发展，在阅读时可能已经过时，并会在未来进行更新。

## 路线图跟踪器

| 升级 | URGE | 轨道 | 主题 | 交叉引用 |
| :-----: | :---------: | :-------: | :-------------------------------: | :--------------------------------------------------------------------------------------------------------------------------------: |
| ET | The Scourge | MEV 轨迹 | 终局之战区块生产流程 | 与以下交集：[IL](/wiki/research/inclusion-lists.md)、[ePBS](/wiki/research/PBS/ePBS.md)、[PEPC](/wiki/research/PBS/PEPC.md) |



## 相关背景
最近的提案和发展旨在实现 Proposer-Builder Separation ([PBS](/wiki/research/PBS/pbs.md))，旨在减少执行区块构建对一些中心化的链下实体 (中继) 的依赖，在当前的背景下 MEV，充当验证者、载荷提议者和专门的、更复杂的区块构建者之间的中介。今天，随着 [MEV-boost](/wiki/research/PBS/mev-boost.md)，验证者-as- 提议者放弃了构建执行载荷的权利，在公开竞价无需许可的拍卖中，构建者提供了提议者付款时的有序载荷。
...
APS 设计原理与**ePBS - 供奉的 Proposer-Builder Separation**设计空间密切相关。
...
Execution Tickets 与 ePBS 正交，从某种意义上说，正在提出一种新的范例： 
验证者/证明者–提议者分离，其中提议者由协议招募，并可以再次将构建角色委托给另一个实体构建者。
然而，Beacon block 的交付仍然是验证者的特权。
...
当提议者尽可能延迟提出区块以便最大化其可以获得的价值时，就会发生计时游戏。在区块-auction ePBS 中，由于执行载荷是在 Beacon block 发布时确定的，因此验证者将玩计时游戏，它可能会尝试尽可能多地延迟 Beacon block 的交付，以便承诺尽可能高的出价。
...
* 用于选择 Beacon block 提议者和 证明者的抽签。这是现有的 PoS 抽签，其中随机选择验证者来提议 Beacon block。在 Execution Tickets 下，Beacon block 将不再包含执行载荷 (区块的已执行交易的最终列表)，而是包含一个 Inclusion Lists，它指定执行中必须存在的一组交易区块。
...
1. 在信标回合中，随机选择的信标提议者有权提议 Beacon block。
2. 这个提议者提出了包含 Inclusion Lists 的 Beacon block。
3. 信标证明者对 Beacon block 的有效性和及时性进行投票。
...
- C1。从抵抗审查的角度来看，我们可能希望在执行载荷的构建中保持验证者的活跃状态。 
  ...
  - 反驳反驳 C1：
    ...
    CC1。今天，当他们使用 MEV-Boost 时，验证者完全放弃了这种能力，并且当他们使用 ePBS 时，很可能完全放弃这种能力。 Inclusion Lists [[6]](#resources) 和 Multiplicity gadgets [[7]](#resources) 等改进建议可以通过对拥有 Execution Tickets 的任何人施加对载荷构造具有绑定但不降低效率的约束来解决此问题。
...
- C3。将验证者和 提议者角色分开会创建一种市场结构，其中持票人可以成为“主动垄断者”，实施 [垄断定价](https://arxiv.org/abs/2311.12731)，并可能对时间敏感的流量进行抢先交易。上述情况与 MEV-Boost 下的现状形成对比，其中验证者作为提议者角色中的*被动垄断者* [[5]](#resources)。
  ...
  - 反驳反驳 C3：
    ...
    CC3。 Quintus 和 Conor 在他们的著作 *何时出售您的区块* [[5]](#resources) 中指出，如果验证者从长远来看倾向于理性，则验证者-as- 提议者和 Ticket-holders-as- 提议者可能会减少很多，这意味着验证者-提议者角色分离的改进可能来自分离本身可以给 ET 以及随后为协议带来的好处，并且这种角色分离很可能不会造成比现状更糟糕的情况。

## 未决问题

Execution Tickets 正在积极研究中，一些已知的未解决问题是：

* ET 市场的具体设计以及门票定价机制背后的机制是什么？

* Execution Tickets 的分叉选择有何影响？
    * 同一个时隙中的 fork-choice 中可能有多个有效条目
    * 载荷模棱两可导致头部分割视图

* 执行票如何改变 Preconfirmations 的设计
...
[[4] ethresearch 关于 Execution Tickets 的帖子](https://ethresear.ch/t/execution-tickets/17944)
