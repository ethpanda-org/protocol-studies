...
[Mev-boost](https://github.com/flashbots/mev-boost) 是提议者-构建者分离 (PBS) 的广泛认可的协议外开源实现。它允许验证者将 区块构建外包给专门的构建者，这可能会增加验证者奖励并提高网络效率。
...
一方面，mev-boost 实现了以太坊节点使用的 [构建者 API](https://github.com/ethereum/builder-specs)，以外包区块的生产。另一方面，它连接到中继的网络并处理构建者和 提议者之间的通信。
...
3. **提议者选择：**
   验证者运行连接到其信标节点的 mev-boost 软件。当选择验证者来提议区块时，他们会从中继接收区块，并根据预定义的标准选择最好的一个，通常是提供最高奖励的区块。
...
- 构建者持续监视交易池 (内存池) 中是否有新的交易。他们根据潜在的 MEV 机会评估这些交易。他们选择最符合其 MEV 优化标准的交易。此外，区块构建者可以从私人订单流或 MEV 搜索者中获取交易捆绑包，就像矿工在原始 Flashbots 拍卖中的 PoW 以太坊中所做的那样。在后一种情况下，构建者接受搜索者的密封价格出价，并将其捆绑包包含在区块中。
- 一旦选择了交易，构建者将它们组装成区块，确保区块遵守以太坊协议的规则，例如：例如，txs 有效，gas 限制未超过。
...
- **减少中心化：** Mev-boost 能够打造更具竞争力的区块建筑景观，降低大型矿池的规模经济，并使家庭质押者获得同样的奖励。
...
总体而言，mev-boost 代表着在以太坊中实现 PBS 的潜在优势迈出了重要一步。然而，持续的研究和开发对于应对挑战并确保安全、分散和高效的实施至关重要。实现更稳定的 PBS 模型的一种途径是[将其纳入协议](/wiki/research/PBS/ePBS.md)，将类似 mev-boost 的功能直接添加到以太坊客户端中。  
