# Consensus Layer (CL) 架构

> 许多区块链共识协议都是_forkful_。分叉链使用分叉选择规则，有时会进行重组。
>
> Ethereum 的共识协议结合了两个独立的共识协议。 _LMD GHOST_ 本质上提供了活力。 _Casper FFG_ 提供最终确定性。他们一起被称为_Gasper_。
>
> 在_live_ 协议中，总会有好事发生。在_安全_协议中，不会发生任何不好的事情。任何实用的协议都不可能永远安全且永远有效。
...
- **Proof-of-Work**：在 Ethereum 和 Bitcoin 中，使用“最重链规则”(有时称为“最长链”，尽管不严格准确)。头区块是完成最多累积“工作”的链的尖端。
> 请注意，与普遍看法相反，Ethereum 的 Proof-of-Work 协议 [未在其分叉选择中使用](https://ethereum.stackexchange.com/questions/38121/why-did-ethereum-abandon-the-ghost-protocol/50693#50693) 任何形式的 GHOST。这种误解非常持久，可能是由于 [Ethereum 白皮书](https://ethereum.org/en/whitepaper/#modified-ghost-implementation)。最终，当 Vitalik 被问及此事时，他证实，虽然 GHOST 是在 PoW 下计划的，但由于担心一些未指明的攻击而从未实施。最重链式法则更简单且经过充分测试。效果很好。
- **Casper FFG (Proof-of-Stake)**：在 Ethereum 的 PoS Casper FFG 协议中，分叉选择规则是“遵循包含**最大高度**的合理检查点的链”并且永远不会恢复最终确定的状态区块。
- **LMD GHOST (Proof-of-Stake)**：在 Ethereum 的 PoS LMD GHOST 协议中，分叉选择规则是采用“贪婪最重观察子树”。它涉及计算验证者对 区块及其后代区块的累积投票。它还应用与 Casper FFG 相同的规则。

每个分叉选择规则都会为区块分配一个数字分数。获胜的区块，或头区块，得分最高。目标是所有正确的节点，当他们看到某个区块时，都会同意它是头并遵循它的分支。这样，所有正确的节点最终将就一条可追溯到创世纪的规范链达成一致。

#### 重组和回归

当节点收到新投票 (以及 Proof-of-Stake 中区块的新投票) 时，它会使用此新信息重新评估分叉选择规则。通常，新的区块将是当前头区块的子级，并且它将成为新的头区块。

然而，有时，新的区块可能是区块树中不同区块的后代。如果节点没有新区块的父区块，它将向其对等节点以及任何其他缺失的区块询问。

在更新后的区块树上运行分叉选择规则可能会显示新头区块与之前的头区块位于不同的分支上。当这种情况发生时，节点必须执行 reorg (重组)。这意味着它将删除 (恢复) 之前包含的区块并在新头的分支上采用区块。
...
Ethereum 的共识协议旨在在良好的网络条件下提供安全性和活跃性。但是，它会在网络出现问题时优先考虑活性。在网络分区中，每一侧的节点将继续生成区块，但不会实现最终确定性 (安全属性)。如果分裂持续下去，各方可能会最终确定不同的历史，从而导致两条不可调和的独立链条。

因此，虽然 Ethereum 努力实现安全性和活跃性，但它倾向于确保网络保持活跃并继续处理交易，即使以严重网络中断期间潜在的安全问题为代价。

## The Ghosts in the Machine

Ethereum 的 Proof-of-Stake 共识协议结合了两个独立的协议：[LMD GHOST](/wiki/CL/gasper?id=lmd-ghost.md) 和 [Casper FFG](/wiki/CL/gasper?id=casper-ffg.md)。它们共同形成了称为“Gasper”的共识协议。有关这两种协议以及它们如何组合工作的详细信息将在下一节 [Gasper](/wiki/CL/gasper) 中介绍。
...
本质上，LMD GHOST 保持链向前发展，而 Casper FFG 通过最终确定区块来确保稳定性。这种组合允许 Ethereum 优先考虑活跃度，这意味着即使 Casper FFG 无法最终确定区块，链也会继续增长。尽管这种组合机制并不总是完美的并且具有一定的复杂性，但它是一种实用的工程解决方案，在 Ethereum 的实践中效果良好。

## 架构

Ethereum 是 节点的去中心化网络，通过对等节点到 对等节点连接进行通信。这些连接是由运行 Ethereum 的 客户端软件的计算机形成的。
...
- **Beacon node**：Beacon node 使用客户端软件来协调 Ethereum 的 Proof-of-Stake 共识。示例包括 Prysm、Teku、Lighthouse 和 Nimbus。Beacon node 与其他 Beacon node、本地执行节点以及可选的本地验证者进行通信。

- **验证者**：验证者客户端是一个允许人们在 Ethereum 的 Consensus Layer 中质押 32 个 ETH 的软件。 验证者在 Proof-of-Stake 系统中提出区块，取代了 Proof-of-Work 矿工。 验证者仅与本地 Beacon node 通信，该信标指示它们并将其工作广播到网络。

托管实际应用程序的主要 Ethereum 网络称为 Ethereum 主网。 Ethereum 主网是 Ethereum 的实时生产实例，它铸造和管理真实的 Ethereum (ETH) 并持有真实的货币价值。

还有测试网络为开发人员、节点运行者创建和管理测试 Ethereum，以及验证者，以便在主网上使用真正的 ETH 之前测试新功能。每个 Ethereum 网​​络都有两层：Execution Layer (EL) 和 Consensus Layer (CL)。每个 Ethereum 节点都包含两层软件：Execution Layer 客户端软件 (如 Nethermind、Besu、Geth 和 Erigon) 和 Consensus Layer 客户端软件 (如 Prysm、Teku、Lighthouse、Nimbus 和 Lodestar)。
...
**Consensus Layer** 负责维护共识链 (Beacon Chain) 并处理从其他对等节点收到的共识区块 (Beacon block) 和证明。 **共识客户端** 参与一个单独的 [点对点网络](/wiki/CL/cl-networking.md)，其规范与执行客户端不同。他们需要参与区块八卦才能从对等节点接收新的区块并在轮到他们求婚时广播区块。
...
2. 预先验证区块。
3. 将区块中的交易作为执行载荷发送到 Execution Layer。
4. Execution Layer 执行交易并验证区块状态。
5. Execution Layer 将验证数据发送回 Consensus Layer。
6. Consensus Layer 将区块添加到其区块链中并对其进行证明，通过网络广播证明。

**当共识客户端是区块生产者时：**
1. 收到成为下一个区块生产人的通知。
2. 在执行客户端时调用 create 区块方法。
3. Execution Layer 访问交易内存池。
4. 执行客户端将交易捆绑到区块中，执行它们，并生成区块哈希。
5. 共识客户端将 交易和 区块哈希添加到 Beacon block 中。
...
如果 $S$ 是 Beacon state 并且 $B$ 是 Beacon block，则状态转换函数 $f$ 是：
...
与区块驱动的 Proof-of-Work 不同，Beacon Chain 是 时隙驱动的。状态更新取决于时隙进度，无论区块是否存在。

Beacon Chain 的 状态转换函数包括：

1. **每时隙转换**：$S' \equiv f_s (S)$
2. **每区块转换**：$S' \equiv f_b (S, B)$
3. **每 epoch 转换**：$S' \equiv f_e (S)$

每个函数都会在特定时间更新链，如 Beacon Chain 规范中所定义。
...
- [Vitalik 带注释的 Ethereum 2.0 规范](https://notes.ethereum.org/@vbuterin/SkeyEI3xv)
- Ethereum，[“Eth2：带注释的规范”](https://github.com/ethereum/annotated-spec)
- Martin Kleppmann，[分布式系统](https://www.youtube.com/playlist?list=PLeKd45zvjcDFUEv_ohr_HdUFe97RItdiB)
- Leslie Lamport 等人，[Byzantine Generals' Problem](https://lamport.azurewebsites.net/pubs/byz.pdf)
- 奥斯汀·格里菲斯，[拜占庭将军 - ETH.BUILD。](https://www.youtube.com/watch?v=c7yvOlwBPoQ)
- 迈克尔·斯普劳尔，[“Ethereum 内部”](https://www.youtube.com/watch?v=LviEOQD9e8c) 
- [Ben Edgington 的 Eth2 手册](https://eth2book.info/capella/part2/consensus/)
- [Lighthouse 共识客户端架构](https://www.youtube.com/watch?v=pLHhTh_vGZ0)
