...
## 概述 
[**Kurtosis**](https://docs.kurtosis.com/) 是一个开发和测试平台，旨在高效打包和启动容器化服务环境。 Kurtosis 作为多容器测试环境的构建系统，帮助开发人员设置可以在本地轻松复制的以太坊网络实例。 [**以太坊包**](https://github.com/ethpandaops/ethereum-package) 构建于 Kurtosis 之上，支持使用 Docker 或 Kubernetes 快速设置可定制、可扩展的私有以太坊测试网。它支持所有主要的执行层 (EL) 和共识层 (CL) 客户端，有效管理本地端口映射和服务连接，以验证和测试以太坊核心基础设施。

本文简要介绍了 Kurtosis、如何安装和使用它，以及使用以太坊开发网的一些基本命令。
...
您可以按照[此处](https://docs.kurtosis.com/install)的官方安装指南安装Kurtosis。
...
有关升级说明，请参阅[Kurtosis升级指南](https://docs.kurtosis.com/upgrade)。
...
您可以按照 [ethereum-package GitHub 页面](https://github.com/kurtosis-tech/ethereum-package) 中的说明，使用以太坊包快速设置默认以太坊开发网。
...
Kurtosis中的**enclave**是一个部署服务的隔离环境。它允许开发者创建多个开发网而不会受到干扰。关键命令包括：
...
您可以克隆以太坊软件包仓库并在其之上进行开发。要运行您自己的修改，请使用以下命令：
...
Kurtosis 提供了几个内置工具来与以太坊开发网进行交互。以下是一些示例：

- [**dora**](https://github.com/ethpandaops/dora)：一个轻量级的信标链浏览器。
...
- [以太坊包 GitHub 仓库](https://github.com/kurtosis-tech/ethereum-package)
