...
引擎 API 是 Consensus Layer (CL) 和 Execution Layer (EL) 之间经过身份验证的通信接口，在 The Merge 中引入。 CL 通过此接口驱动 EL 的区块构建、验证和分叉选择。它告诉 EL 哪条链是规范的，要求它构建新的区块，并发送收到的区块进行验证。
...
CL 和 EL 共享一个 256 位 (32 字节) 十六进制编码的 JWT 密钥，通过指向 `--jwt-secret` 文件的 `jwt.hex` CLI 标志进行配置。如果省略，EL 会自动为该会话生成一个随机密钥，并将其写入其数据目录中的 `jwt.hex`。
...
一种可选方法，允许 CL 和 EL 识别彼此的软件。每一方返回一个 `ClientVersionV1` 结构，其中包含两个字母的客户端代码、人类可读的名称、版本字符串和 4 字节提交哈希。紧凑的标识符旨在适合 32 字节 Beacon block 涂鸦字段，用于网络多样性跟踪。
...
将执行载荷从接收到的 Beacon block 传送到 EL 进行验证。

**参数：**
- `executionPayload`：完整的区块
- `expectedBlobVersionedHashes`：blob 版本哈希的有序列表
- `parentBeaconBlockRoot`：父 Beacon block 根 (EIP-4788)
...
检索使用 `forkchoiceUpdated` 调用 `payloadAttributes` 后构建的载荷和 EL。

**参数：** `payloadId` 是 `engine_forkchoiceUpdatedV3` 返回的 8 字节 ID。

**返回：**
- `executionPayload`：组装后的区块
- `blockValue`：`feeRecipient`的优先费中的总 wei(256 位数量)
- `blobsBundle`：`{commitments, proofs, blobs}`。包含 48 字节 KZG 承诺、48 字节 KZG 证明和 131,072 字节 blob 数据，用于 CL 构建 blob sidecar。
...
在 V4 (Prague/Electra) 中引入，执行请求允许 EVM 智能合约触发 Consensus Layer 状态更改。每个请求都是 `request_type ++ request_data`，其中 `request_type` 是 1 字节前缀。
...
| `0x01` | EIP-7002 | **提现请求**：智能合约持有`0x01`提现凭证可以触发验证者部分或全部退出 EVM。通过 `0x00000961Ef480Eb55e80D19ad83579A64c007002` 处的预部署进行处理。目标：2/ 区块；最大值：16/ 区块。费用：`fake_exponential(1_wei, excess, 17)`，正常负载下接近 1wei，持续需求下呈指数级昂贵。系统调用 gas：30,000,000 (不计入区块 gas 记账)。 |
...
| 版本 | EL 分叉 | CL 分叉 | 关键变化 |
|---------|---------|---------|-------------|
| V1 | Paris | Bellatrix | 初始合并后：`forkchoiceUpdated`、`newPayload`、`getPayload` |
| V2 | Shanghai | Capella | 将 `withdrawals` 添加到载荷属性和执行载荷 |
...
| V4 | Prague | Electra | 添加 `executionRequests` (EIP-7685) |
...
| 时间 | 行动 |
|------|--------|
| **t = 0s** | 时隙开始。 CL 使用 `engine_forkchoiceUpdatedV3` 调用 `payloadAttributes`。 EL 返回 `payloadId`。 |
...
在重负载期间，CL 可以乐观地导入 Beacon block，而无需等待完整的 EVM 执行。如果 EL 稍后返回 `INVALID`，则 CL 会重新组织回 `latestValidHash`。最大乐观导入深度以 `SAFE_SLOTS_TO_IMPORT_OPTIMISTICALLY` 为界 (默认值：**128 时隙**，约 25.6 分钟)，可通过 `--safe-slots-to-import-optimistically` 进行配置。这可以防止“分叉选择中毒”攻击，其中恶意对等节点在链尖端提供结构有效但计算无效的区块。
...
- **EIP-7892 (BPO 分叉)**：仅调整 blob 参数的分叉允许 blob 计数在 Fusaka 后通过轻量级网络调整进行扩展，而无需触发完整的 hard fork。 BPO1 和 BPO2 在 Fusaka 之后不久激活，将 blob 目标分别提高到 10/15 和 14/21。

**执行性能：**
- **EIP-7935**：默认区块 gas 限制提高到 **60M** (在客户端之间协调，不是共识规则)。
- **EIP-7825**：交易 gas 限制上限为 **2^24 = 16,777,216 gas**。任何一个交易都不能占据整个区块。
- **EIP-7934**：RLP 执行区块大小上限为 **8 MiB** (`MAX_RLP_BLOCK_SIZE = 10 MiB - 2 MiB safety margin`)。 CL 八卦协议已经将区块丢弃超过 10 MiB； 2 MiB 余量为 Beacon block 保留余量，使有效 EL 载荷限制为 8 MiB。
...
**EIP-7732 (ePBS)** 是拟议的 Glamsterdam CL 头条新闻。它将 PBS 从协议外 MEV-Boost sidecar 移至 Consensus Layer，消除了对可信中继的依赖。引擎 API 时隙生命周期发生重大变化：提议者在 t=3s 时仅接收签名的 **构建者承诺** 并在没有执行载荷的情况下广播它；然后，构建者在 t=4s 证明截止日期后向网络显示完整的 `ExecutionPayloadEnvelope`。这可以防止提议者窃取构建者的 MEV，同时保留证明能力。新引擎 API 方法与完整的 EVM 执行分开处理承诺验证。
...
- [EIP-3675：升级到 Proof-of-Stake](https://eips.ethereum.org/EIPS/eip-3675)
...
- [EIP-7607：hard fork 元 - Fusaka](https://eips.ethereum.org/EIPS/eip-7607)
- [EIP-7732：供奉 Proposer-Builder Separation](https://eips.ethereum.org/EIPS/eip-7732)
- [EIP-7928：区块级别访问列表](https://eips.ethereum.org/EIPS/eip-7928)
- [Engine API 视觉指南](https://hackmd.io/@danielrachi/engine_api)
