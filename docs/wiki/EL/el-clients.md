...
> **执行客户端**，以前称为 *eth1 客户端*，正在实现以太坊 [执行层](https://github.com/ethereum/execution-specs)，其任务是处理和广播交易并管理状态。
他们通过 p2p 接收交易，使用 [以太坊虚拟机](https://ethereum.org/en/developers/docs/evm/) 运行每个交易的计算以更新状态并确保遵循协议规则。 
...
| [Geth](https://github.com/ethereum/go-ethereum) |Go | 以太坊基金会 |生产|
...
| [JS 客户端](https://github.com/ethereumjs/ethereumjs-monorepo) |TypeScript | 以太坊基金会 |开发|
| [ethrex](https://github.com/lambdaclass/ethrex) |Rust| LambdaClass |开发|
| [Akula](https://github.com/akula-bft/akula) |Rust|Akula开发商|已弃用 |
...
| [三位一体](https://github.com/ethereum/trinity) |蟒蛇 |开放以太坊 |已弃用 |


## 分布

目前，绝大多数节点操作员都使用 Geth 作为执行客户端。 
为了支持执行层的健康，[建议在运行节点时使用不同的客户端](https://clientdiversity.org/#why)。 
...
Erigon 的架构允许其在三天内完成完整存档节点同步，数据存储量少于 3 TB，使其成为运行此类节点的理想选择。它还包含自己的嵌入式CL 客户端，使其能够独立运行。 
...
作为以太坊的原始 Go 实现，也是最早积极维护的客户端，Geth (Go-以太坊) 在开发人员和用户中得到了广泛采用。
...
Nethermind 适用于私有以太坊网络和去中心化应用程序(dApp)开发，强调数据完整性和性能可扩展性。
...
Reth (Rust 以太坊) 是一个模块化、高效的以太坊客户端，专为用户友好性和高性能而设计。受 Erigon 设计的启发，它围绕新颖的存档节点方法构建，可在较小的磁盘占用空间内实现快速同步。
...
Nimbus 作为超轻量级以太坊执行层客户端专注于效率和安全性。最初致力于 Nimbus CL 客户端，一个新团队在 Nim 中开发了一个执行客户端。 
它最大限度地减少了资源消耗，同时支持以太坊的 执行层功能并与 Fluffy(门户网络轻客户端)集成。
Nimbus 提供增强的内存节省和状态同步机制，非常适合资源受限的环境。

### 蚕
Silkworm是以太坊执行层协议的C++实现，旨在成为最快的以太坊客户端。
