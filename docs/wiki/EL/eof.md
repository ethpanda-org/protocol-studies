...
目前，[EVM 字节码](/wiki/EL/evm.md#evm-bytecode) 是一个非结构化指令序列。 EOF 引入了容器的概念，为字节码带来了结构，使得 EVM 更容易解析和分析。其主要目标是通过允许在部署时进行一次性代码验证以及严格分离代码与数据来实现对 EVM 的重大升级。
...
- [**EIP-5920**](https://eips.ethereum.org/EIPS/eip-5920)：引入 qz​​ph0000082qz 操作码 (`0xFC`)，允许在不调用接收者代码的情况下进行 ETH 传输，从而降低重入风险。
...
- [**EIP-7620**](https://eips.ethereum.org/EIPS/eip-7620)：用 `CREATE` 和 `CREATE2` 替换 `EOFCREATE`/`RETURNCODE`。

- [**EIP-7873**](https://eips.ethereum.org/EIPS/eip-7873)：替换以前的 EIP-7698。定义 `TXCREATE` 操作码和 InitcodeTransaction 以从 EOA 直接进行 EOF 部署。
...
| 特色 | 旧版 EVM | EOF |
|---|---|---|
| Container 结构 | 非结构化字节码 | 明确定义的部分 (类型、代码、数据) |
...
EOF 于 2025 年 4 月 28 日在 [互操作测试#34 调用](https://ethereum-magicians.org/t/interop-testing-34-april-28-2025/23822/2) 期间正式从 Fusaka hard fork 中删除。 Tim Beiko 在 [GitHub PR 更新 EIP-7607](https://github.com/ethereum/EIPs/pull/9703) 中确认了这一决定。

被删除的主要原因是：

- **复杂性：** 批评者，包括开发者 Pascal Caversaccio 在 [2025 年 3 月 Ethereum Magicians 帖子](https://ethereum-magicians.org/t/evm-object-format-eof/5727) 中，认为 EOF 引入了太多同时发生的更改、新语义、十多个操作码添加和删除，并且可以通过更小、更有针对性的更新来实现其好处相反。
...
EOF 的冠军们有机会提出纳入后续 **Glamsterdam** hard fork 的案例，但目前尚未计划纳入。该规范及其组成的 EIP 仍可作为历史参考。
...
- [**Ethereum 魔术师：EVM 对象格式 (EOF)**](https://ethereum-magicians.org/t/evm-object-format-eof/5727) — 原始的 EIP 讨论线程，其中 EOF 是由核心开发人员提出并辩论的。

- [**Ethereum 注释：EOF**](https://notes.ethereum.org/t-1tLFnLSKCtLZpb-Rw0IA) — 有关 EOF 规范和开放设计问题的社区注释和工作文档。
