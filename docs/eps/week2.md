# 第 3 讲 | 学习小组 Execution Layer

第三天，我们将深入 Ethereum 的 Execution Layer。 
...
* [Ethereum：力学](https://cs251.stanford.edu/lectures/lecture7.pdf)(基于这些幻灯片的讲座也可以在 YouTube 上找到：[Ethereum Execution Layer 概述 - Dan Boneh](https://www.youtube.com/watch?v=7sxBjSfmROc))

## 概要

###  Execution Layer 节点概述
* 区块验证
    * 用过于简单的术语来说，EL 处理状态转换
    * 每个交易都由客户端验证、执行，其结果累积到状态 Trie 中
    * 还有其他机制也必须每个区块进行更新，例如 EIP-1559 基本费用、EIP-4844 超额 blob gas、EIP-4844 信标根环缓冲区、Beacon Chain 提款等。
...
* 标头验证
    ...
    * 验证时间戳
* 区块验证
    * `Process(..)` 中的演练 [`state_processor.go`](https://github.com/ethereum/go-ethereum/blob/master/core/state_processor.go#L60)

### EVM 高级
* 堆垛机介绍
* 看看简单的程序
* 查看操作码的不同类别：堆栈/内存操纵器、env getter、Ethereum 系统操作等。
...
- [通过研究 Ethereum 的源码来理解 Ethereum](https://gisli.hamstur.is/2020/08/understanding-ethereum-by-studying-the-source-code/)
