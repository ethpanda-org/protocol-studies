...
节点通过点对点网络使用**gossip协议**广播交易。这些交易会根据特定的标准（例如，检查**nonce**的正确性、足够的余额和有效的签名）进行验证，并存储在**内存池**中，等待被包含到区块中。

---

每个时隙都有一个指定的区块提议者，通过共识层的伪随机过程选出。当一个验证者被选为某个时隙的区块提议者时，它的共识客户端通过执行引擎的**fork choice updated**方法启动区块构建过程，该方法提供了构建区块所需的上下文。

我们可以简化并模拟区块构建的过程，尽管这种方法特定于geth中使用的Go类型。但这些概念通常可以应用于不同的客户端。
